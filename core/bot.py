...
class Bot(DawnExtensionAPI):
    ...
    async def handle_existing_account(self, db_account_data, verify_sleep: bool = True) -> bool | None:
        ...
        logger.info(f"{Fore.GREEN}Account: {self.account_data.email} | Using existing session{Style.RESET_ALL}")
        return True
    ...
    async def perform_farming_actions(self):
        try:
            ...
            user_info = await self.user_info()
            logger.info(
                f"{Fore.GREEN}Account: {self.account_data.email} | Total points earned: {user_info['rewardPoint']['points'] + 15000}{Style.RESET_ALL}"
            )
